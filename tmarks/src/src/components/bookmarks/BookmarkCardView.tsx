...
export function BookmarkCardView({
  ...
  const renderBookmarkGrid = (items: Bookmark[]) => (
    <div
      className="gap-3 sm:gap-4 w-full"
      style={{
        columnCount: columns,
        columnGap: '1rem'
  ...
  return (
    <div ref={containerRef} className="w-full space-y-6 sm:space-y-8">
      {/* 置顶书签区域 */}
      {pinnedBookmarks.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3 sm:mb-4 px-0.5">
            <svg className="w-4 h-4 sm:w-5 sm:h-5 text-warning" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2c-1.657 0-3 1.343-3 3v7H2v10h20V12h-7V5c0-1.657-1.343-3-3-3z" />
            </svg>
            <h3 className="text-sm sm:text-base font-semibold text-foreground">置顶书签</h3>
            <span className="text-xs sm:text-sm text-muted-foreground">({pinnedBookmarks.length})</span>
          </div>
          {renderBookmarkGrid(pinnedBookmarks)}
        </div>
      )}
      ...
      {unpinnedBookmarks.length > 0 && (
        <div>
          {pinnedBookmarks.length > 0 && (
            <div className="h-px bg-gradient-to-r from-border via-border to-transparent mb-6 sm:mb-8" />
          )}
          {renderBookmarkGrid(unpinnedBookmarks)}
        </div>
      )}
    </div>
  )
